...
void Command_Analysis::Receive_Data_Analysis(void)
{
	...
	case Non_existent: Serial.println("不存在于本设备的帧ID");break;//异常处理
	///*卷膜机私有指令*/
...
void Command_Analysis::Set_General_controller_output_init(void)
{
	...
	if (Verify_Frame_Validity(4, gReceiveCmd[3], true, false) == true)//第4个参数选择了false，不校验工作组号
	{
		...
		unsigned char DO_Init[8] = { 0x00 };unsigned char AO_Init[16] = { 0x00 };
		...
		iwdg_feed();
		delay(1000);//等待回执
		unsigned char R_Modbus_Instructions[10]; unsigned int R_Modbus_Length = 0;
